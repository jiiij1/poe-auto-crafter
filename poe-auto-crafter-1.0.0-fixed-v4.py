...
DEFAULT_CONFIG = {
    ...
    "copy_timeout": 0.1,              # 单次等待剪贴板变化超时（秒）
    "copy_poll_interval": 0.005,      # 剪贴板轮询间隔（秒）
    "copy_retry": 3,                  # 复制失败重试次数
    "pre_copy_delay": 0.034,           # 发送复制快捷键前稳定等待（秒）
    "post_action_delay": 0.05,        # 动作执行后稳定等待（秒）
    "post_drop_delay": 0.01,          # 左键放下装备后稳定等待（秒）
    "read_failure_threshold": 0.1,    # 判定长时间读取不到剪贴板的阈值（秒）
...
def wait_clipboard_change(base_text="", timeout=0.6, poll_interval=0.005):
    """
    等待剪贴板从 base_text 变成非空文本。

    这里专门用于 POE 装备复制：调用方会在发送 Ctrl+C 前先把剪贴板
    清空，因此只有真正的 Ctrl+C 把文本写入剪贴板后才会返回。
    """
    if pyperclip is None:
        return None
    ...
    while time.perf_counter() - start < timeout:
        ...
        if text is not None:
            text = str(text)
            if text != base_text and text.strip():
                return text
...
def copy_item_text(
    ...
    每次 Ctrl+C 前先清空剪贴板，再发送复制快捷键，并等待剪贴板从空
    变成非空文本。这样不会因为 Ctrl+C/游戏处理较慢而误读上一次动作
    留在剪贴板中的装备文本。

    返回：本次复制得到的非空文本；失败返回 None。
    """
    if pyperclip is None:
        return None

    for attempt in range(max(1, int(retries))):
        # 关键：先清空剪贴板，建立一个明确的“本次复制前”状态。
        # 不再把旧装备文本作为有效结果。
        if not safe_clipboard_copy("", retries=2, delay=0.005):
            time.sleep(0.01)
            continue

        # 确认剪贴板确实已经为空，避免清空失败后马上读取到旧文本。
        cleared = False
        clear_deadline = time.perf_counter() + 0.03
        while time.perf_counter() < clear_deadline:
            current = safe_clipboard_paste(retries=1, delay=0)
...
class PoeClipboardMakerApp:
    ...
    def automation_loop(self, rules):
        ...
        copy_timeout = float(self.config.get("copy_timeout", 0.1))
        copy_poll_interval = float(self.config.get("copy_poll_interval", 0.005))
        copy_retry = int(self.config.get("copy_retry", 2))
        copy_backend = self.config.get("copy_backend", "keyboard")

        pre_copy_delay = float(self.config.get("pre_copy_delay", 0.034))
        post_action_delay = float(self.config.get("post_action_delay", 0.05))
        post_drop_delay = float(self.config.get("post_drop_delay", 0.01))

        log_every = int(self.config.get("log_every", 10))
        verbose_log = bool(self.config.get("verbose_log", False))
        read_failure_threshold = float(self.config.get("read_failure_threshold", 0.1))
    ...
    def _interruptible_sleep(self, seconds, interval=0.005):
        """可被停止状态及时打断的 sleep，避免动作停止后还继续等待。"""
            ...
            while self.is_running() and current and current != "end":
                ...
                if action_name:
                    ...
                    if not ok:
                        ...
                        break
                    if post_action_delay > 0:
                        end_delay = time.perf_counter() + post_action_delay
                        while self.is_running():
                            remaining = end_delay - time.perf_counter()
                            if remaining <= 0:
                                break
                ...
                if rules:
                    ...
                    self.ui_queue.put(("log", f"  开始读取剪贴板 timeout={copy_timeout:.3f}s"))
                    clipboard_text = copy_item_text(
                        copy_hotkey=copy_hotkey,
                        timeout=copy_timeout,
                        poll_interval=copy_poll_interval,
                    ...
                    self.ui_queue.put(("log", f"  剪贴板读取完成，耗时 {copy_elapsed:.4f}s，结果={'成功' if clipboard_text is not None else '失败'}"))
                    ...
                    for i, rule_entry in enumerate(rules):
                        ...
                        if result["success"]:
                            affix_log = f"，词缀 {result['affix_count']}/{result['affix_count_target']}" if result['affix_count_enabled'] else ""
                            self.ui_queue.put(("log", f"  规则{i+1}「{rule_name}」匹配成功（耗时 {match_elapsed:.4f}s）: 蓝色 {result['blue_count']}/{result['min_blue_match']}{affix_log}"))
                            current = rule_entry.get("success", "end")
                            matched = True
                            # 只有成功分支明确指向 end 时才立即停止整个流程；
                            # 指向其他节点时保留原有的动作拼接/流程跳转功能。
                            if current == "end":
                                self.ui_queue.put(("log", "  匹配成功，立即停止流程（硬停止）"))
                                self.set_running(False)
                                return
                            break
                        else:
