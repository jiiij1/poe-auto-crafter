...
DEFAULT_CONFIG = {
    ...
    "copy_timeout": 0.1,              # 单次等待剪贴板序列号变化超时（秒）
    "copy_poll_interval": 0.005,      # 剪贴板轮询间隔（秒）
    "copy_retry": 3,                  # 复制失败重试次数
    "post_action_delay": 0.05,        # 动作执行后稳定等待（秒），原 pre_copy_delay 已合并至此
...
def copy_item_text(
    ...
    """
    可靠读取 POE 当前装备文本（Clipboard Sequence Number 版本）。

    流程：
    1. 记录当前剪贴板序列号
    2. 发送 Ctrl+C
    3. 轮询等待序列号变化（比清空-确认-轮询文本更高效）
    4. 序列号变化后，读取一次剪贴板内容
    5. 如果内容非空且有效，返回文本

    优势：
    - 不需要清空剪贴板，省去 30ms+ 的清空+确认延迟
    - 即使两次装备文本完全相同，也能检测到新的 Ctrl+C
    - 减少 pyperclip.paste() 调用次数
...
class PoeClipboardMakerApp:
    ...
    def automation_loop(self, rules):
        ...
        # 合并后的单一延迟：动作执行后等待游戏稳定
        # 原来 post_action_delay(50ms) + pre_copy_delay(30ms) = 80ms
        # 现在只保留一个 post_action_delay，默认 30ms 即可
